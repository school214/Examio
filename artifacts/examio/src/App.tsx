...
import {
  getGetAdminSummaryQueryKey, getGetAttemptQueryKey, getGetDashboardQueryKey,
  ...
  useGetDashboard, useGetExam, useGetResult, useListAdminExams, useListExams,
  useListResults, useSaveAttempt, useStartAttempt, useSubmitAttempt, useUpdateAdminExam,
} from '@workspace/api-client-react';
import type { AdminExam, ExamSummary, LearningProfile, Question, ResultSummary } from '@workspace/api-client-react';
...
function ProfileCard({ profile }: { profile: LearningProfile }) {
  return <Card className="overflow-hidden" data-testid="card-learning-profile"><div className="flex items-center justify-between bg-primary px-6 py-5 text-primary-foreground"><div><p className="font-mono text-[10px] uppercase tracking-[.17em] text-primary-foreground/55">Din lärprofil</p><h2 className="mt-1 text-lg font-semibold">{profile.subject}</h2></div><Sparkles size={20} className="text-accent" /></div><div className="p-6"><p className="text-sm font-semibold text-secondary-foreground">{profile.focusLabel}</p><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{profile.focusText}</p><div className="mt-7 space-y-4">{profile.dimensions.map((dimension) => <div key={dimension.label} data-testid={`metric-dimension-${dimension.label}`}><div className="mb-1.5 flex justify-between text-xs"><span className="font-medium">{dimension.label}</span><span className="font-mono text-muted-foreground">{Math.round(dimension.score)}%</span></div><div className="h-2 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-secondary-foreground transition-all duration-700" style={{ width: `${Math.min(100, dimension.score)}%` }} /></div></div>)}</div></div></Card>;
}
...
function AdminPage() {
  ...
  const update = useUpdateAdminExam();
